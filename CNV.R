...
runInferCNV <- function(SeuratObject,
  ...
  p.colors <- colorRamp2(c(0.9, 0.98, 1.02, 1.1), c("blue", "white", "white", "red"))
  spot.color <- list(Cluster = cur.color)
  gene.color <- list(CHR = chr.color)
  spot.ha <- rowAnnotation(df = spot.anno,
                           col = spot.color,
                           show_annotation_name = F)
  gene.ha <- HeatmapAnnotation(df = gene.anno,
                               col = gene.color,
...
runCopyKAT <- function(SeuratObject,
  ...
  copykat.test <- copykat::copykat(rawmat=as.matrix(SeuratObject@assays$Spatial@counts),
                                   id.type=id.type,
                                   ngene.chr=ngene.chr,
                                   win.size=win.size,
                                   KS.cut=KS.cut,
