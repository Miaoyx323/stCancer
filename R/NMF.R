...
runNMF <- function(object,
    ...
    prog.colors <- colorRampPalette(c("white", "yellow", "red"))
    ...
    for(p.i in  1 : length(mat.colnames)){
        ps[[p.i]] <- SpatialFeature_Plot(object,
                                         features = mat.colnames[[p.i]],
                                         legend.name = colnames(p.H)[[p.i]],
                                         legend.color = prog.colors,
