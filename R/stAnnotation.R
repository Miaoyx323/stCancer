...
stAnnotation <- function(object,
  ...
  clusters <- sort(unique(object@meta.data[[paste0("SCT_snn_res.", clst.resolution)]]))
  cluster.colors <- c("#FEC643", "#437BFE", "#43FE69", "#FE6943", "#E78AC3",
                      "#43D9FE", "#FFEC1A", "#A6D854", "#E5C494", "#7F6699",
                      "#C643FE", "#333399", "#679966")
  if(length(cluster.colors) < length(clusters)){
    cluster.colors <- getDefaultColors(n = length(clusters), type = 2)
  }
  cluster.colors <- cluster.colors[1:length(clusters)]
  names(cluster.colors) <- as.character(clusters)

  meta.color <- as.numeric(as.character(
    object@meta.data[[paste0("SCT_snn_res.", clst.resolution)]]))
  ...
  cluster.csv <- data.frame(Barcode = colnames(object),
                            Cluster = object@meta.data[[paste0("SCT_snn_res.", clst.resolution)]])
...
#   saveRDS(object, file.path(savePath_data, "spatial_stat.RDS"))
  ...
  if(bool.interaction){
    ...
    results.interaction <- SpatialInteraction(SeuratObject =  object,
